...
function buildPublicLocal(fqdn) {
  ...
  var hash = 'my-link-box'; // until I get my glue records for useraddress.net right
...
function proxyPublicLocal(hash) {
  // serve a web server on the local network:
  https.createServer({
    key: fs.readFileSync('scripts/certs/my-link-box.useraddress.net/privkey.pem'),
    cert: fs.readFileSync('scripts/certs/my-link-box.useraddress.net/cert.pem'),
    ca: fs.readFileSync('scripts/certs/my-link-box.useraddress.net/chain.pem')
  }, (req, res) => {
    proxy.web(req, res, { target: `http://localhost:${ports.backend}` });
...
buildSelfSigned().then(fqdn => {
  return buildPublicLocal(fqdn).then(hash => {
    ...
    console.log('calling qrGen', fqdn);
    return qrGen(fqdn);
  }).then(() => {
